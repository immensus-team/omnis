...
describe("0005_memory", () => {
  ...
  it("indexes only live memories with HNSW cosine", async () => {
    ...
    await query(
      pool,
      `INSERT INTO memories (content, embedding, kind) VALUES ('logan prefers tea', $1::vector, 'preference')`,
      [unit768],
    );
    // ponytail: the integration tests share one omnis_test DB, and files that ran earlier insert
    // then delete hundreds of rows in memories. HNSW leaves dead elements in the graph, so
    // ef_search (default 40) can scan only dead elements, stop, and miss the one live row (the
    // scan returns 0 rows). VACUUM sweeps the dead index entries, so the measurement is the same
    // no matter what earlier files did.
    ...
    expect(hit[0]?.content).toContain("tea");
