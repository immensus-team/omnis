...
const GOOD = ["--- a/VOICE.md", "+++ b/VOICE.md", "@@ -1 +1,2 @@", " hello", "+Hello, boss"].join(
  "\n",
);
...
describe("self-model apply (A4 §13.2)", () => {
  ...
  // audit_log.target_id is a uuid (0006_kernel.sql) — putting a file name there kills this insert.
  it("commits the patch and records the audit row the approval executor needs", async () => {
    ...
    try {
      const { commit } = await applyApprovedSelfModelPatch(
        { file: "VOICE.md", diff: GOOD, rationale: "always address the boss formally" },
        { audit: createAudit(pool), approvalId },
      );
      expect(commit).toMatch(/^[0-9a-f]{40}$/);
      expect(readFileSync(join(dir, "VOICE.md"), "utf8")).toContain("Hello, boss");
  ...
  it("refuses a stale diff before @omnis/memory touches the repo", async () => {
    ...
    expect(readFileSync(join(dir, "VOICE.md"), "utf8")).toBe("hello\n");
  });
});
