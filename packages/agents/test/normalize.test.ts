...
// The Korean strings in this file are the Korean-language attack samples the scanner exists to
// catch (A4 §11.2-A) and the fixtures `normalizeExternal` must keep normalising. Keep them
// verbatim — translating one would change what is being tested.
...
describe("normalizeExternal — 3. no base64/hex decoding", () => {
...
describe("normalizeExternal — 5. tag escape and truncation", () => {
  ...
  it("keeps head 4000 and tail 2000 with a marker when longer than 8000", () => {
    // Hangul filler syllables stand in for any long body text (head / middle / tail).
...
describe("scanInjection (A4 §11.2-A → the five flags of delta §4)", () => {
