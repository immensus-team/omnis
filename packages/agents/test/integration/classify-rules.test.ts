...
import { WORK_DOMAINS, applyRules } from "../../src/classify/rules.js";
...
beforeAll(async () => {
  ...
  const t = await pool.query<{ id: string }>(
    `INSERT INTO threads (account_id, external_id, kind, scope) VALUES ($1,'thr_rules','email','unknown')
       ON CONFLICT (account_id, external_id) DO UPDATE SET scope='unknown' RETURNING id`,
    [accountId],
  );
...
describe("applyRules", () => {
  ...
  it("returns null when nothing matches, so the kNN stage runs", async () => {
    const hit = await applyRules(item({ author_person_id: null }), {
      threadId,
      accountChannel: "telegram",
      pool,
    });
    expect(hit).toBeNull();
  });
