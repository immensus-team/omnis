...
describe("WhatsApp contract: fixture replay", () => {
  for (const [file, fixture] of fixtures) {
    it(`${fixture.scenario} (${file})`, () => {
      if (fixture.expected.errorKind) {
        expect(mapApiError(fixture.raw).kind).toBe(fixture.expected.errorKind);
        // A Beeper error envelope is not a payload either — the same raw must not sneak past
        // normalize() and put a row in the inbox.
        expect(normalize(fixture.raw)).toEqual([]);
      } else {
...
describe("WhatsApp contract: NormalizedItem invariants", () => {
  ...
  for (const { file, index, item } of produced) {
    it(`${file}#${index} → ${item.externalId ?? "(no externalId)"}`, () => {
      ...
      // A1 §2.6 maps the Beeper message id to both `externalId` and `sourceHash` — the kernel's
      // idempotency is keyed on the latter, so the two drifting apart would break dedupe silently.
      ...
      if (item.threadMeta !== undefined) {
        ...
        // The item's thread and its threadMeta have to agree, or the kernel would file the item under
        // one external id and upsert a thread row under another.
