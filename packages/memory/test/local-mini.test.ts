...
describe("scanRoots", () => {
  ...
  it("walks subdirectories and returns absolute paths with mtime and size", async () => {
    ...
    await writeFile(join(root, "b.md"), "note B");
  ...
  it("skips files over the 2MB cap", async () => {
    // Frozen hangul filler: each "가" is 3 bytes in UTF-8, so repeating it
    // MAX_INGEST_FILE_BYTES times comfortably exceeds the byte cap (the check is a
    // strict `size > cap`, so an ASCII filler would have to be length cap + 1 instead).
...
/** macOS's fs.watch(recursive) arms the FSEvents stream asynchronously and does not replay
 *  events from before the stream started. A single write can be lost entirely, so keep writing
...
describe("watchLocalRoots", () => {
  it("reports a changed file and stops reporting after the returned unsubscribe", async () => {
    ...
    try {
      ...
      await writeUntilSeen(join(root, "watched.md"), "new file", sawWatched);
      expect(sawWatched()).toBe(true);
    } finally {
      stop();
    }
    const before = seen.length;
    await writeFile(join(root, "after-stop.md"), "should be ignored");
  ...
  it("never reports a denied path", async () => {
    ...
    const stop = watchLocalRoots({ roots: [root], logger, onChange: (p) => seen.push(p) });
    try {
      // Write .env only after the watcher is really armed — otherwise this test passes vacuously.
      const sawOk = (): boolean => seen.some((p) => p.endsWith("ok.md"));
      await writeUntilSeen(join(root, "ok.md"), "fine", sawOk);
