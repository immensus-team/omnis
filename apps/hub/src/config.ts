export interface HubConfig {
  ...
  host: "127.0.0.1";
  version: string;
  /** The A6 wrapper injects the value of the Keychain item omnis.bridge.token.<host>. Empty = WS /bridge is closed. */
  bridgeToken: string;
  /** The Zero token `sub`. The permissions compare against this value (packages/kernel/src/zero-schema.ts). */
  userId: string;
  /** Same value as zero-cache's ZERO_AUTH_SECRET. Empty = /api/zero-token returns 503. */
...
/** Master §4.2: the hub binds 127.0.0.1:8787 only. Tailscale Serve exposes it under /api/. */
