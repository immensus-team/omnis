 ...
 *
 *  It is *not* up on a fresh 390 load any more, and that is main's round-1 rule rather than a change
 *  in this wave: `paneVisible` carries a `!floating` term (App.tsx, NC-03/L-04), so the seed's
 *  pending approval no longer opens the narrow pane unasked — at 390 the tier asks for it through
 *  the subline's approval count. The helper is kept because the drawer it puts down is a *modal*
 *  one: `pointer-events: none` on everything outside itself, so once a page has asked for the pane
 *  the whole shell — rail tiles, filters, compose — is behind its scrim until it is dismissed, which
 *  is why every sequence that presses chrome at this tier calls this first. Dismissal sticks
 *  (`paneDismissed`), so one Escape is enough per page. */
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // The pane does not survive the narrowing on its own, and that is main's round-1 rule rather
    // than a missing gesture: `paneVisible` carries a `!floating` term (App.tsx, NC-03/L-04), so a
    // queue-opened column — which is what the divider above was dragging — gives way to the list
    // when the window crosses 1280, and the tier has to be asked for a sheet. This is a fixture
    // step, not an assertion: what is checked below is the drawer's box, not how it was asked for.
    // A press on a row is the gesture that names a *thread*, which is what this still documents.
    if ((await page.locator("[data-vaul-drawer].app-shell__detail").count()) === 0) {
      await page.locator(".inbox-row").first().click();
      await page.locator("[data-vaul-drawer].app-shell__detail").waitFor({ timeout: 15_000 });
      await page.waitForTimeout(800);
    }
    ...
    // proposes, and at 1440 App.tsx auto-opens the pane's column on it (`approvals.length > 0` with
    // nothing selected) — a column that covers the list the row has to be hovered in. (Measured
    // before the merge that brought in the `!floating` term above: at 390 the hover timed out for
    // 30s on `<section data-testid="detail-pane"> intercepts pointer events`; that tier no longer
    // auto-opens, and the check is kept for both so the two sequences measure the same list.)
    // Closing it (state 'expired' + decision 'ignore' keeps the row and satisfies
    // approvals_decided_ck — the same fixture adjustment shots-responsive.ts makes) is what makes
    // the list reachable.
    await query(
      pool,
