...
// The chip strip is deliberately given a real chip first: an empty strip has nothing to scroll, so
// the assertion would pass even if the row had gone back to wrapping.
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // are reached by the loop below anyway.
    ...
    // Put one label filter on, so the strip actually has to hold pills + archived + chip + trigger.
    ...
    // Park the pointer somewhere inert before shooting. Clicking the option left the virtual mouse
    // over the first inbox row, which opens that row's hover card — it lands off the left edge at
    // 1440px and shows up in the screenshot as a stray panel over the rail. The ask bar has no
    // hover-only affordance, so hovering it changes nothing.
    ...
    for (const width of WIDTHS) {
      ...
      await page.screenshot({ path: join(OUT, `${width}.png`) });
      ...
      console.log(
        `width ${width}: overflow ${over}px, filter row ${filterRow.toFixed(1)}px, ${railPart}`,
      );
