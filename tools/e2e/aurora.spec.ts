...
test.describe("US-D06 aurora surface map", () => {
  test("the shell's aurora, and nothing else", async ({ page }) => {
    ...
    await check("§5.2's mobile gate, plus no aurora box past the edge", async () => {
      const notes: string[] = [];
      for (const width of [320, 375, 390, 414, 768]) {
        ...
        expect(overflow.diff, describeOverflow(overflow)).toBe(0);
        const stray = await page.evaluate(readSurfaceOverflow);
        expect(stray, `aurora surface past the edge at ${String(width)}px`).toEqual([]);
  ...
  test("the onboarding preview's void surface", async ({ page }) => {
    ...
    await check("onboarding does not scroll sideways at 390px", async () => {
      await page.setViewportSize({ width: 390, height: 844 });
      await page.waitForTimeout(120);
      const overflow = await page.evaluate(measureOverflow);
      assertNoOverflow("390px (onboarding)", overflow);
      return describeOverflow(overflow);
    });
  });
