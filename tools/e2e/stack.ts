// Process/DB stack for the Phase A end-to-end smoke. It lives outside the workspace (tools/) and is
// only ever invoked through the root `pnpm e2e:phase-a` (= tsx tools/e2e/run.ts) — not production code.
//
// What it brings up: the omnis_e2e DB (migrations + Zero permissions) → zero-cache (4848)
// → hub (8787) → Vite desktop dev server (5173). The local agent bridge is attached to
// WS /bridge from inside this same process by seed.ts.
...
/** Contract §8: this smoke's bridge connects as the macbook host. */
...
/** Creates tools/e2e/.env when missing. The four secrets are one-shot random values for this smoke only (never printed). */
export function loadOrCreateEnv(): E2EEnv {
  if (!existsSync(ENV_FILE)) {
    ...
    writeFileSync(
      ENV_FILE,
      [
        "# Generated by: tools/e2e/stack.ts. One-shot local smoke only — never committed (.env is gitignored).",
...
/** Idempotent: drop and recreate if present. Two consecutive runs must produce the same result (run.ts's idempotence check). */
export async function resetDatabase(env: E2EEnv): Promise<void> {
  ...
  try {
    ...
    // zero-cache needs REPLICATION rights. This smoke runs on a single superuser (the one-shot startup procedure in ops/zero-cache.env.example).
...
export function startDesktop(): void {
  start("desktop", "pnpm", ["--filter", "@omnis/desktop", "dev"], {
    ...process.env,
    // Empty string = relative path → the vite dev proxy forwards to the hub (apps/desktop/vite.config.ts).
    OMNIS_HUB_HTTP_URL: "",
  });
}

/** With an earlier run or another worktree holding the port we would silently mistake it for "already up". */
