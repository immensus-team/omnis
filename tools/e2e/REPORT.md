...
Generated: 2026-09-21T15:22:57.349Z · `pnpm e2e:phase-a` (tools/e2e/run.ts)

Stack: PostgreSQL `omnis_test_merge_w2_e2e` (migrations 0001–0008 + Zero permissions) → zero-cache :4848
...
| PASS | no aurora inside a message body, thread item or draft card (§4.2) | 61ms | 0 message bodies tinted |
...
| PASS | screen 1 (nothing selected) | 783ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 2 (ask panel open) | 1053ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 4 (agents filter) | 764ms | 320px: overflow 0px, worst svg > rect +114.8px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px, worst svg > rect +59.8px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px, worst svg > rect +44.8px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px, worst svg > rect +20.8px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
...
| PASS | A9 ⌘K opens the floating AI panel and types into the command list | 29ms | panel + cmdk list reachable by typing |
| FAIL | A-archive archiving a row removes it from the list and restoring brings it back | 20059ms | expect(received).toBe(expected) // Object.is equality | Expected: 7 | Received: 6 | Call Log: | - Timeout 20000ms exceeded while waiting on the predicate |
| FAIL | G5 a new item reaches the UI in ≤2s | 20048ms | expect(locator).toBeVisible() failed | Locator: getByRole('option').filter({ hasText: 'G5 latency probe 1790004091142' }) | Expected: visible | Timeout: 20000ms | Error: element(s) not found | Call log: |
| PASS | A8b hub recorded audit_log(approval.decided) | 3ms | 1 row(s) |
...
## Pass 2 (65.6s, items=14)

| Result | Check | Time | Note |
| --- | --- | --- | --- |
| PASS | only the rail carries an aurora when the ask panel is closed | 5ms | 1 surface, variant=mist |
| PASS | no aurora inside a list row (§4.2) | 4ms | 0 list rows tinted |
| PASS | opening the ask panel adds exactly one dawn surface | 420ms | 2 surfaces: rail + aurora ask-panel |
| PASS | every rendered aurora mass still computes blur + drop-shadow (§2.4) | 2ms | 1 mass(es) blurred and rimmed; 1 hidden by variant |
...
| PASS | screen 2 (ask panel open) | 1162ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 4 (agents filter) | 781ms | 320px: overflow 0px, worst svg > rect +114.8px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px, worst svg > rect +59.8px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px, worst svg > rect +44.8px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px, worst svg > rect +20.8px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | onboarding is a single void surface | 2ms | 1 surface, variant=void |
| PASS | every word sits on the scrim card, never on the aurora (§5.3 guard 4) | 1ms | 0 text nodes outside the card |
| PASS | onboarding does not scroll sideways at the four widths (§5.2) | 639ms | 320px: overflow 0px; 375px: overflow 0px; 390px: overflow 0px; 414px: overflow 0px; 768px: overflow 0px |
...
| PASS | the pane's bar clears the header at rest at 900px (§c.5) | 5ms | 0.0px clear, chrome needs 0px |
| PASS | no glass layer sits on another at 899px (§4.4/§e guard 4) | 2ms | 5 glass layers, 0 nested |
...
| PASS | A5 Thread screen renders seeded items with status badges | 46ms | 4 status badges |
| PASS | A6 Agent Session screen shows turns and a ToolCallBadge | 35ms |  |
| PASS | A7 Approval card shows the pending approval | 43ms |  |
...
| FAIL | A-archive archiving a row removes it from the list and restoring brings it back | 20061ms | expect(received).toBe(expected) // Object.is equality | Expected: 7 | Received: 6 | Call Log: | - Timeout 20000ms exceeded while waiting on the predicate |
| FAIL | G5 a new item reaches the UI in ≤2s | 20059ms | expect(locator).toBeVisible() failed | Locator: getByRole('option').filter({ hasText: 'G5 latency probe 1790004156893' }) | Expected: visible | Timeout: 20000ms | Error: element(s) not found | Call log: |
