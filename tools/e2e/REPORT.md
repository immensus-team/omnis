...
Generated: 2026-09-21T15:31:03.223Z · `pnpm e2e:phase-a` (tools/e2e/run.ts)
...
| PASS | opening the ask panel adds exactly one dawn surface | 294ms | 2 surfaces: rail + aurora ask-panel |
| PASS | every rendered aurora mass still computes blur + drop-shadow (§2.4) | 1ms | 1 mass(es) blurred and rimmed; 1 hidden by variant |
| PASS | no aurora inside a message body, thread item or draft card (§4.2) | 59ms | 0 message bodies tinted |
...
| PASS | screen 1 (nothing selected) | 772ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 2 (ask panel open) | 1036ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 4 (agents filter) | 766ms | 320px: overflow 0px, worst svg > rect +114.8px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px, worst svg > rect +59.8px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px, worst svg > rect +44.8px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px, worst svg > rect +20.8px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
...
| PASS | onboarding does not scroll sideways at the four widths (§5.2) | 630ms | 320px: overflow 0px; 375px: overflow 0px; 390px: overflow 0px; 414px: overflow 0px; 768px: overflow 0px |
| PASS | the row's hover cluster is what a click lands on at 1440px (§c.7) | 52ms | a click lands on button.inbox-row__action, opacity 1, pointer-events auto |
...
| PASS | no glass layer sits on another at 1280px (§4.4/§e guard 4) | 3ms | 4 glass layers, 0 nested |
...
| PASS | no glass layer sits on another at 900px (§4.4/§e guard 4) | 3ms | 3 glass layers, 0 nested |
| PASS | the pane's bar clears the header at rest at 900px (§c.5) | 4ms | 0.0px clear, chrome needs 0px |
...
| PASS | A9 ⌘K opens the floating AI panel and types into the command list | 38ms | panel + cmdk list reachable by typing |
| FAIL | A-archive archiving a row removes it from the list and restoring brings it back | 20068ms | expect(received).toBe(expected) // Object.is equality | Expected: 7 | Received: 6 | Call Log: | - Timeout 20000ms exceeded while waiting on the predicate |
| FAIL | G5 a new item reaches the UI in ≤2s | 20058ms | expect(locator).toBeVisible() failed | Locator: getByRole('option').filter({ hasText: 'G5 latency probe 1790004577099' }) | Expected: visible | Timeout: 20000ms | Error: element(s) not found | Call log: |
...
## Pass 2 (65.5s, items=14)

| Result | Check | Time | Note |
| --- | --- | --- | --- |
| PASS | only the rail carries an aurora when the ask panel is closed | 3ms | 1 surface, variant=mist |
| PASS | no aurora inside a list row (§4.2) | 3ms | 0 list rows tinted |
| PASS | opening the ask panel adds exactly one dawn surface | 419ms | 2 surfaces: rail + aurora ask-panel |
| PASS | every rendered aurora mass still computes blur + drop-shadow (§2.4) | 2ms | 1 mass(es) blurred and rimmed; 1 hidden by variant |
| PASS | no aurora inside a message body, thread item or draft card (§4.2) | 59ms | 0 message bodies tinted |
| PASS | every aurora is one of §3.2's three surfaces, outside §4.2's list | 2ms | 2 surface(s) checked against the §3.2 allow-list |
| PASS | no element carries both .aurora and .glass-surface (§2.7) | 3ms | 0 collisions |
| PASS | no aurora, or its ancestor, carries a filter (§2.4) | 1ms | every wrapper computes filter: none, no filtered ancestor |
| PASS | the glass inside an aurora is still glass (§4.4) | 1ms | 2 glass plate(s) blurring inside an aurora |
| PASS | screen 1 (nothing selected) | 768ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 2 (ask panel open) | 1153ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 4 (agents filter) | 789ms | 320px: overflow 0px, worst svg > rect +114.8px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px, worst svg > rect +59.8px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px, worst svg > rect +44.8px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px, worst svg > rect +20.8px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | onboarding is a single void surface | 2ms | 1 surface, variant=void |
| PASS | every word sits on the scrim card, never on the aurora (§5.3 guard 4) | 0ms | 0 text nodes outside the card |
| PASS | onboarding does not scroll sideways at the four widths (§5.2) | 623ms | 320px: overflow 0px; 375px: overflow 0px; 390px: overflow 0px; 414px: overflow 0px; 768px: overflow 0px |
...
| PASS | no glass layer sits on another at 390px (§4.4/§e guard 4) | 6ms | 5 glass layers, 0 nested |
| PASS | the D9 chrome surfaces are the shared glass, with no fill of their own | 2ms | .sheet×1, .bottom-bar__piece×2 |
| PASS | the sheet arrives without moving under reduced motion (§c.6) | 305ms | 500ms travel → 0ms under reduce (animation none) |
| PASS | the detail body measures 15px/1.5 at 1440px (§b.4) | 3ms | 15px / 22.5px |
| PASS | the detail body measures 17px/1.5 at 390px (§b.4) | 1ms | 17px / 25.5px |
...
| PASS | A2d a conversation row has avatar + name + relative time + summary | 5ms | time="now" summary="Draft: Yes, I will review it today.…" |
| PASS | A4 work/personal filter pills change the list | 99ms | all=8 work=[#omnis-launch] personal=[Dana Lee] |
| PASS | A4b channel rail tile filters the list, Inbox tile restores it | 67ms | all=8 gmail=2 |
| PASS | A5 Thread screen renders seeded items with status badges | 45ms | 4 status badges |
| PASS | A6 Agent Session screen shows turns and a ToolCallBadge | 35ms |  |
| PASS | A7 Approval card shows the pending approval | 42ms |  |
| PASS | A8 Approve → hub moves the approval to decided | 426ms | pending → decided, through §c.8's prompt |
| PASS | A9 ⌘K opens the floating AI panel and types into the command list | 36ms | panel + cmdk list reachable by typing |
| FAIL | A-archive archiving a row removes it from the list and restoring brings it back | 20065ms | expect(received).toBe(expected) // Object.is equality | Expected: 7 | Received: 6 | Call Log: | - Timeout 20000ms exceeded while waiting on the predicate |
| FAIL | G5 a new item reaches the UI in ≤2s | 20065ms | expect(locator).toBeVisible() failed | Locator: getByRole('option').filter({ hasText: 'G5 latency probe 1790004642760' }) | Expected: visible | Timeout: 20000ms | Error: element(s) not found | Call log: |
| PASS | A8b hub recorded audit_log(approval.decided) | 1ms | 1 row(s) |
| PASS | A8c pending_approvals.state moved to decided(accept) | 0ms | state=decided decision=accept |
| PASS | A10 classify() recorded a T0 run in agent_runs (no network) | 0ms | 1 run(s), tier=T0 |
| PASS | A11 local-agent registered over WS /bridge | 0ms | agent_runtimes state=offline |
