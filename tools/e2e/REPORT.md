...
생성: 2026-09-20T14:53:07.816Z · `pnpm e2e:phase-a` (tools/e2e/run.ts)
...
## Pass 1 (9.8s, items=14)

| 결과 | 검증 | 소요 | 비고 |
| --- | --- | --- | --- |
| PASS | A1 Inbox lists one row per seeded thread (U2: 행이 item이 아니라 thread 단위) | 14ms | 8 thread rows (item count was 14) |
...
| PASS | A4 work/personal filter pills change the list | 123ms | all=8 work=[#omnis-launch] personal=[Dana Lee <dana@example.com>] |
...
| PASS | A-archive 행 보관 → 목록에서 사라지고, 되살리면 돌아온다 | 325ms | "omnis launch sync" archived → restored (8 rows), audit_log 2종 기록 |
| PASS | G5 a new item reaches the UI in ≤2s | 73ms | 28ms ingest → 화면 (목표 ≤2000ms) |
...
| PASS | A-archive 행 보관 → 목록에서 사라지고, 되살리면 돌아온다 | 325ms | "omnis launch sync" archived → restored (8 rows), audit_log 2종 기록 |
| PASS | G5 a new item reaches the UI in ≤2s | 69ms | 27ms ingest → 화면 (목표 ≤2000ms) |
| PASS | A8b hub recorded audit_log(approval.decided) | 1ms | 1 row(s) |
| PASS | A8c pending_approvals.state moved to decided(accept) | 0ms | state=decided decision=accept |
| PASS | A10 classify() recorded a T0 run in agent_runs (no network) | 0ms | 1 run(s), tier=T0 |
| PASS | A11 local-agent registered over WS /bridge | 0ms | agent_runtimes state=online |
