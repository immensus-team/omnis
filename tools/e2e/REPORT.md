...
생성: 2026-09-20T13:51:35.121Z · `pnpm e2e:phase-a` (tools/e2e/run.ts)
...
| PASS | A-archive 행 보관 → 목록에서 사라지고, 되살리면 돌아온다 | 327ms | "omnis launch sync" archived → restored (8 rows), audit_log 2종 기록 |
| PASS | G5 a new item reaches the UI in ≤2s | 72ms | 27ms ingest → 화면 (목표 ≤2000ms) |
...
## Pass 2 (7.6s, items=14)

| 결과 | 검증 | 소요 | 비고 |
| --- | --- | --- | --- |
| PASS | A1 Inbox lists one row per seeded thread (U2: 행이 item이 아니라 thread 단위) | 12ms | 8 thread rows (item count was 14) |
| PASS | A2 Inbox rows show a visible channel icon for all three channels | 21ms | Slack 메시지 / Gmail 메시지 / Google Calendar 메시지 |
...
| PASS | A4 work/personal filter pills change the list | 94ms | all=8 work=[#omnis-launch] personal=[Dana Lee <dana@example.com>] |
...
| PASS | A6 Agent Session screen shows turns and a ToolCallBadge | 28ms |  |
| PASS | A7 Approval card shows the pending approval | 10ms |  |
| PASS | A8 Approve → hub moves the approval to decided | 40ms | pending → decided |
| PASS | A9 ⌘K opens the command palette | 15ms |  |
| PASS | A-archive 행 보관 → 목록에서 사라지고, 되살리면 돌아온다 | 327ms | "omnis launch sync" archived → restored (8 rows), audit_log 2종 기록 |
| PASS | G5 a new item reaches the UI in ≤2s | 74ms | 26ms ingest → 화면 (목표 ≤2000ms) |
