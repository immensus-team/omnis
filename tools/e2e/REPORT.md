...
생성: 2026-09-20T11:27:11.999Z · `pnpm e2e:phase-a` (tools/e2e/run.ts)
...
## Pass 1 (9.9s, items=13)

| 결과 | 검증 | 소요 | 비고 |
| --- | --- | --- | --- |
| PASS | A1 Inbox lists one row per seeded thread (U2: 행이 item이 아니라 thread 단위) | 13ms | 7 thread rows (item count was 13) |
| PASS | A2 Inbox rows show a visible channel icon for all three channels | 37ms | Slack 메시지 / Gmail 메시지 / Google Calendar 메시지 |
...
| PASS | A2c kinso shell: channel rail tiles + ask/search bar | 13ms | rail: Inbox/Slack/Gmail/Google Calendar/Agent + ask bar |
| PASS | A2d a conversation row has avatar + name + relative time + summary | 7ms | time="now" summary="초안: 네, 오늘 중으로 리뷰할게요.…" |
| PASS | A4 work/personal filter pills change the list | 137ms | all=7 work=[#omnis-launch] personal=[PoC slides] |
...
| PASS | A9 ⌘K opens the command palette | 17ms |  |
| PASS | A-archive 행 보관 → 목록에서 사라지고, 되살리면 돌아온다 | 325ms | "omnis launch sync" archived → restored (7 rows), audit_log 2종 기록 |
...
| PASS | A4 work/personal filter pills change the list | 94ms | all=7 work=[#omnis-launch] personal=[PoC slides] |
| PASS | A4b channel rail tile filters the list, Inbox tile restores it | 68ms | all=7 gmail=2 |
...
| PASS | A-archive 행 보관 → 목록에서 사라지고, 되살리면 돌아온다 | 341ms | "omnis launch sync" archived → restored (7 rows), audit_log 2종 기록 |
| PASS | G5 a new item reaches the UI in ≤2s | 67ms | 29ms ingest → 화면 (목표 ≤2000ms) |
