...
Generated: 2026-09-21T22:20:24.565Z · `pnpm e2e:phase-a` (tools/e2e/run.ts)
...
| PASS | opening the ask panel adds exactly one dawn surface | 291ms | 2 surfaces: rail + aurora ask-panel |
...
| PASS | no aurora, or its ancestor, carries a filter (§2.4) | 4ms | every wrapper computes filter: none, no filtered ancestor |
| PASS | the glass inside an aurora is still glass (§4.4) | 1ms | 2 glass plate(s) blurring inside an aurora |
| PASS | screen 1 (nothing selected) | 795ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 2 (ask panel open) | 1051ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 4 (agents filter) | 828ms | 320px: overflow 0px, worst svg > rect +129.7px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px, worst svg > rect +74.7px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px, worst svg > rect +59.7px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px, worst svg > rect +35.7px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
...
| PASS | the row's hover cluster is what a click lands on at 1440px (§c.7) | 44ms | a click lands on button.inbox-row__action, opacity 1, pointer-events auto |
...
| PASS | the pane's bar clears the header at rest at 1280px (§c.5) | 2ms | 28.0px clear, glass needs 20px |
...
| PASS | no glass layer sits on another at 1024px (§4.4/§e guard 4) | 3ms | 4 glass layers, 0 nested |
...
| PASS | the pane's bar clears the header at rest at 900px (§c.5) | 2ms | 0.0px clear, chrome needs 0px |
| PASS | no glass layer sits on another at 899px (§4.4/§e guard 4) | 1ms | 4 glass layers, 0 nested |
| PASS | no glass layer sits on another at 768px (§4.4/§e guard 4) | 2ms | 4 glass layers, 0 nested |
| PASS | no glass layer sits on another at 390px (§4.4/§e guard 4) | 5ms | 4 glass layers, 0 nested |
...
| PASS | A2 Inbox rows show a visible channel icon for all three channels | 30ms | Slack message / Gmail message / Google Calendar message |
...
| PASS | A5 Thread screen renders seeded items with status badges | 48ms | 3 status badges, folded draft card |
...
| FAIL | A-archive archiving a row removes it from the list and restoring brings it back | 20096ms | expect(received).toBe(expected) // Object.is equality | Expected: 7 | Received: 6 | Call Log: | - Timeout 20000ms exceeded while waiting on the predicate |
| FAIL | G5 a new item reaches the UI in ≤2s | 20059ms | expect(locator).toBeVisible() failed | Locator: getByRole('option').filter({ hasText: 'G5 latency probe 1790029137728' }) | Expected: visible | Timeout: 20000ms | Error: element(s) not found | Call log: |
...
| PASS | opening the ask panel adds exactly one dawn surface | 282ms | 2 surfaces: rail + aurora ask-panel |
| PASS | every rendered aurora mass still computes blur + drop-shadow (§2.4) | 2ms | 1 mass(es) blurred and rimmed; 1 hidden by variant |
| PASS | no aurora inside a message body, thread item or draft card (§4.2) | 57ms | 0 message bodies tinted |
| PASS | every aurora is one of §3.2's three surfaces, outside §4.2's list | 2ms | 2 surface(s) checked against the §3.2 allow-list |
...
| PASS | the glass inside an aurora is still glass (§4.4) | 3ms | 2 glass plate(s) blurring inside an aurora |
| PASS | screen 1 (nothing selected) | 790ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 2 (ask panel open) | 1015ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 4 (agents filter) | 791ms | 320px: overflow 0px, worst svg > rect +129.7px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px, worst svg > rect +74.7px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px, worst svg > rect +59.7px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px, worst svg > rect +35.7px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | onboarding is a single void surface | 2ms | 1 surface, variant=void |
| PASS | every word sits on the scrim card, never on the aurora (§5.3 guard 4) | 1ms | 0 text nodes outside the card |
| PASS | onboarding does not scroll sideways at the four widths (§5.2) | 618ms | 320px: overflow 0px; 375px: overflow 0px; 390px: overflow 0px; 414px: overflow 0px; 768px: overflow 0px |
| PASS | the row's hover cluster is what a click lands on at 1440px (§c.7) | 58ms | a click lands on button.inbox-row__action, opacity 1, pointer-events auto |
| PASS | no glass layer sits on another at 1440px (§4.4/§e guard 4) | 2ms | 4 glass layers, 0 nested |
| PASS | the pane's bar clears the header at rest at 1440px (§c.5) | 2ms | 28.0px clear, glass needs 20px |
...
| PASS | the pane's bar clears the header at rest at 1100px (§c.5) | 1ms | 0.0px clear, chrome needs 0px |
| PASS | no glass layer sits on another at 1024px (§4.4/§e guard 4) | 5ms | 4 glass layers, 0 nested |
| PASS | the pane's bar clears the header at rest at 1024px (§c.5) | 2ms | 0.0px clear, chrome needs 0px |
| PASS | no glass layer sits on another at 900px (§4.4/§e guard 4) | 6ms | 3 glass layers, 0 nested |
| PASS | the pane's bar clears the header at rest at 900px (§c.5) | 2ms | 0.0px clear, chrome needs 0px |
| PASS | no glass layer sits on another at 899px (§4.4/§e guard 4) | 2ms | 4 glass layers, 0 nested |
...
| PASS | the sheet arrives without moving under reduced motion (§c.6) | 304ms | 500ms travel → 0ms under reduce (animation none) |
| PASS | the detail body measures 15px/1.5 at 1440px (§b.4) | 3ms | 15px / 22.5px |
...
| PASS | A2b Inbox rows show the seeded thread titles | 2ms | #omnis-launch / omnis launch sync |
| PASS | A3 Inbox rows carry label chips | 2ms |  |
| PASS | A2c kinso shell: channel rail tiles + ask/search bar | 5ms | rail: Inbox/Slack/Gmail/Google Calendar/Agent + ask bar |
...
| PASS | A5 Thread screen renders seeded items with status badges | 50ms | 3 status badges, folded draft card |
| PASS | A6 Agent Session screen shows turns and a ToolCallBadge | 37ms |  |
| PASS | A7 Approval card shows the pending approval | 50ms |  |
| PASS | A8 Approve → hub moves the approval to decided | 427ms | pending → decided, through §c.8's prompt |
| PASS | A9 ⌘K opens the floating AI panel and types into the command list | 37ms | panel + cmdk list reachable by typing |
| FAIL | A-archive archiving a row removes it from the list and restoring brings it back | 20089ms | expect(received).toBe(expected) // Object.is equality | Expected: 7 | Received: 6 | Call Log: | - Timeout 20000ms exceeded while waiting on the predicate |
| FAIL | G5 a new item reaches the UI in ≤2s | 20058ms | expect(locator).toBeVisible() failed | Locator: getByRole('option').filter({ hasText: 'G5 latency probe 1790029204076' }) | Expected: visible | Timeout: 20000ms | Error: element(s) not found | Call log: |
