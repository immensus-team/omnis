...
Generated: 2026-09-21T20:51:05.234Z · `pnpm e2e:phase-a` (tools/e2e/run.ts)
...
| PASS | no aurora inside a message body, thread item or draft card (§4.2) | 46ms | 0 message bodies tinted |
...
| PASS | screen 1 (nothing selected) | 771ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 2 (ask panel open) | 1024ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 4 (agents filter) | 754ms | 320px: overflow 0px, worst svg > rect +129.7px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px, worst svg > rect +74.7px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px, worst svg > rect +59.7px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px, worst svg > rect +35.7px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | onboarding is a single void surface | 3ms | 1 surface, variant=void |
| PASS | every word sits on the scrim card, never on the aurora (§5.3 guard 4) | 1ms | 0 text nodes outside the card |
| PASS | onboarding does not scroll sideways at the four widths (§5.2) | 616ms | 320px: overflow 0px; 375px: overflow 0px; 390px: overflow 0px; 414px: overflow 0px; 768px: overflow 0px |
...
| PASS | no glass layer sits on another at 768px (§4.4/§e guard 4) | 1ms | 4 glass layers, 0 nested |
| PASS | no glass layer sits on another at 390px (§4.4/§e guard 4) | 1ms | 4 glass layers, 0 nested |
...
| PASS | A1 Inbox lists one row per seeded thread (U2: a row is a thread, not an item) | 15ms | 8 thread rows (item count was 14) |
...
| PASS | A5 Thread screen renders seeded items with status badges | 47ms | 3 status badges, folded draft card |
| PASS | A6 Agent Session screen shows turns and a ToolCallBadge | 35ms |  |
| PASS | A7 Approval card shows the pending approval | 40ms |  |
| PASS | A8 Approve → hub moves the approval to decided | 428ms | pending → decided, through §c.8's prompt |
| PASS | A9 ⌘K opens the floating AI panel and types into the command list | 35ms | panel + cmdk list reachable by typing |
| FAIL | A-archive archiving a row removes it from the list and restoring brings it back | 20097ms | expect(received).toBe(expected) // Object.is equality | Expected: 7 | Received: 6 | Call Log: | - Timeout 20000ms exceeded while waiting on the predicate |
| FAIL | G5 a new item reaches the UI in ≤2s | 20044ms | expect(locator).toBeVisible() failed | Locator: getByRole('option').filter({ hasText: 'G5 latency probe 1790023777565' }) | Expected: visible | Timeout: 20000ms | Error: element(s) not found | Call log: |
...
## Pass 2 (66.4s, items=14)

| Result | Check | Time | Note |
| --- | --- | --- | --- |
| PASS | only the rail carries an aurora when the ask panel is closed | 2ms | 1 surface, variant=mist |
| PASS | no aurora inside a list row (§4.2) | 4ms | 0 list rows tinted |
| PASS | opening the ask panel adds exactly one dawn surface | 281ms | 2 surfaces: rail + aurora ask-panel |
| PASS | every rendered aurora mass still computes blur + drop-shadow (§2.4) | 2ms | 1 mass(es) blurred and rimmed; 1 hidden by variant |
| PASS | no aurora inside a message body, thread item or draft card (§4.2) | 58ms | 0 message bodies tinted |
| PASS | every aurora is one of §3.2's three surfaces, outside §4.2's list | 2ms | 2 surface(s) checked against the §3.2 allow-list |
| PASS | no element carries both .aurora and .glass-surface (§2.7) | 1ms | 0 collisions |
| PASS | no aurora, or its ancestor, carries a filter (§2.4) | 0ms | every wrapper computes filter: none, no filtered ancestor |
| PASS | the glass inside an aurora is still glass (§4.4) | 1ms | 2 glass plate(s) blurring inside an aurora |
| PASS | screen 1 (nothing selected) | 763ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 2 (ask panel open) | 1045ms | 320px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
| PASS | screen 4 (agents filter) | 745ms | 320px: overflow 0px, worst svg > rect +129.7px; rail aurora: dissolved, 4/4 layers off; 375px: overflow 0px, worst svg > rect +74.7px; rail aurora: dissolved, 4/4 layers off; 390px: overflow 0px, worst svg > rect +59.7px; rail aurora: dissolved, 4/4 layers off; 414px: overflow 0px, worst svg > rect +35.7px; rail aurora: dissolved, 4/4 layers off; 768px: overflow 0px; rail aurora: dissolved, 4/4 layers off |
...
| PASS | the row's hover cluster is what a click lands on at 1440px (§c.7) | 74ms | a click lands on button.inbox-row__action, opacity 1, pointer-events auto |
| PASS | no glass layer sits on another at 1440px (§4.4/§e guard 4) | 2ms | 4 glass layers, 0 nested |
| PASS | the pane's bar clears the header at rest at 1440px (§c.5) | 1ms | 28.0px clear, glass needs 20px |
...
| PASS | no glass layer sits on another at 390px (§4.4/§e guard 4) | 3ms | 4 glass layers, 0 nested |
...
| PASS | the detail body measures 15px/1.5 at 1440px (§b.4) | 7ms | 15px / 22.5px |
| PASS | the detail body measures 17px/1.5 at 390px (§b.4) | 1ms | 17px / 25.5px |
| PASS | A1 Inbox lists one row per seeded thread (U2: a row is a thread, not an item) | 14ms | 8 thread rows (item count was 14) |
...
| PASS | A9 ⌘K opens the floating AI panel and types into the command list | 35ms | panel + cmdk list reachable by typing |
| FAIL | A-archive archiving a row removes it from the list and restoring brings it back | 20084ms | expect(received).toBe(expected) // Object.is equality | Expected: 7 | Received: 6 | Call Log: | - Timeout 20000ms exceeded while waiting on the predicate |
| FAIL | G5 a new item reaches the UI in ≤2s | 20052ms | expect(locator).toBeVisible() failed | Locator: getByRole('option').filter({ hasText: 'G5 latency probe 1790023844114' }) | Expected: visible | Timeout: 20000ms | Error: element(s) not found | Call log: |
| PASS | A8b hub recorded audit_log(approval.decided) | 23ms | 1 row(s) |
| PASS | A8c pending_approvals.state moved to decided(accept) | 0ms | state=decided decision=accept |
| PASS | A10 classify() recorded a T0 run in agent_runs (no network) | 0ms | 1 run(s), tier=T0 |
| PASS | A11 local-agent registered over WS /bridge | 0ms | agent_runtimes state=offline |
