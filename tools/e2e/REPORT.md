...
생성: 2026-09-20T14:25:25.305Z · `pnpm e2e:phase-a` (tools/e2e/run.ts)
...
| PASS | A2 Inbox rows show a visible channel icon for all three channels | 23ms | Slack 메시지 / Gmail 메시지 / Google Calendar 메시지 |
...
| PASS | A4 work/personal filter pills change the list | 90ms | all=7 work=[#omnis-launch] personal=[PoC slides] |
...
| PASS | A-archive 행 보관 → 목록에서 사라지고, 되살리면 돌아온다 | 339ms | "omnis launch sync" archived → restored (7 rows), audit_log 2종 기록 |
| PASS | G5 a new item reaches the UI in ≤2s | 74ms | 28ms ingest → 화면 (목표 ≤2000ms) |
...
| PASS | A-archive 행 보관 → 목록에서 사라지고, 되살리면 돌아온다 | 339ms | "omnis launch sync" archived → restored (7 rows), audit_log 2종 기록 |
| PASS | G5 a new item reaches the UI in ≤2s | 74ms | 29ms ingest → 화면 (목표 ≤2000ms) |
