...
Supersedes D6's §0 text. Use this, not that one. The block is quoted in the form it shipped: it was
authored here in Korean and `DESIGN-DIRECTION.md` line 6 has carried the English version of the same
sentence since, so this file was the last place holding the other one — this repo is English-only, and
a quoted replacement is not exempt from that. The two are the same sentence, not two drafts:

```
- **Light theme by default** (plain paper canvas + 3.5% film grain; the grid and the peach/mint
  gradient wash are both retired by the 2026-09-21 monotone directive — colour lives only on the
  three aurora surfaces in `ACCENT-DIRECTION.md` and on the single blue accent). Dark is an option,
  and A5's "dark-first" is dropped.
```
...
#### c.1.1 What the motion-OSS wave actually took, and what it left

Added 2026-09-21, rewritten twice the same day. The first version described S3, S4 and S5 as "Replaced" while they were still open, and credited `@use-gesture` with maths `lib/detail-pane.ts` computes itself. The second corrected those but put a *rule* in their place — "`motion` owns the animation layer where its *completion* is reachable in the test environment" — that had been invented mid-wave, that no slice had been judged by before it, and that would have declined S3, S4 and S5 whether or not they worked. What follows is the state of the tree, slice by slice, with the measurement behind each call.

The rule, in its final form:

> **The vetted libraries own the animation layer. `pointer-drag.ts` keeps the gesture layer: the start slop, the touch hold, Escape, click suppression, and the hit-testing a library would otherwise re-derive. A harness that cannot see an animation is a gap in the harness, not a reason the library does not apply — and the gap is closed by `tools/e2e/`, not by declining the port.**

Why the discarded form was wrong, in one paragraph, because it is the kind of rule that reads well and vetoes everything: jsdom has no layout engine and no Web Animations timeline. `Element.prototype.animate` there is a stub this repo's own test setup deliberately never finishes — it exists so the call is a call, not a crash. So almost nothing "completes" in that environment, and an adoption criterion written against completion is a criterion against adoption. It answered "can the current suite see this?" when the question was "is this the right implementation, and what would it take to see it?". S3, S4 and S5 all landed once the second question was asked, and the answer to "what would it take" was one Playwright script.

**Taken: S1, S3, S4, S5, S6.**

- **S1 — the five libraries, and the tier vocabulary.** `motion@13.4.0`, `vaul@1.1.2`, `@use-gesture/react@10.3.1`, `@formkit/auto-animate@0.10.0`, `sonner@2.0.8` (all MIT). `lib/motion.ts` gains `TIER_MS` (`base` 160 / `move` 240 / `panel` 320) and `SPRING`, which mirror the `--dur-*` rungs in `tokens.css` — with a drift test that reads `tokens.css` off disk and fails if the two ladders diverge in either direction — plus `useMotionPrefs()`, which unions `useReducedMotion()` with `prefers-reduced-transparency`. `MotionConfig reducedMotion="user"` is mounted at the root in `main.tsx`, beside the onboarding preview rather than inside `App`, so the one screen rendered *instead of* `App` is not the one place the preference is ignored. The presets stopped being scaffolding when S4 landed: `SPRING.move`, `motionMs` and `useMotionPrefs` are all read by `inbox-row.tsx` (see the vocabulary note below).
- **S3 — the divider drag is `@use-gesture`'s.** `detail-pane.tsx`'s grip binds `useDrag` with `axis: "x"`, `threshold: 6` and `pointer: { capture: false }` — the same gesture `pointer-drag.ts` documents at length (window-level listeners, a 6px slop, no pointer capture, because the element being dragged is the one the drag re-renders) with the library's own filtering in place of ours. The *maths* stayed ours and stayed unit-tested: `lib/detail-pane.ts` is still pure, total, and the single writer of `RUBBER_BAND`, because the arrow keys land on the same `clampDetailWidth` and a second implementation of the limit is how the two drift apart. What changed is who tracks the pointer. Evidence: `divider-drag-1440-{a,b,c,d}` in `docs/design/screens/motion-oss/` — the pane follows the pointer one-for-one inside the limits (240px of travel, a 660px pane), then lags it in the band (480px of travel, 900px asked, 777.6px drawn against a 720px ceiling), then travels back on release (a frame parked at half of `--dur-move` is still strictly past the ceiling), with the shell's `--detail-width` equal to the laid-out box at every frame. **The second half of S3 — the pane's collapse and expand through `AnimatePresence` and a 240ms layout spring — is declined, and this time with a reason rather than the discarded rule's.** The pane's exit is a *width*: `--detail-width`, whose one writer is `clampDetailWidth`'s tested output, which the drag, the arrow keys, the double-click reset and the settings KV all land on. A `layout` port would re-derive that width by measuring the box, so the number would have two authors. And the hold that lets CSS animate the leave already exists — `useClosingSpring` (`lib/motion.ts`), which the command palette and the ask panel share — so `AnimatePresence` would replace a mechanism three surfaces have in common with a second one doing the same job. What is left is the travel itself, and `--dur-move`/`--ease-settle` already express it: `divider-drag-1440-d-release-120ms` is that pair springing the pane back from the band, and `tokens.css` shortens it to 80ms under reduced motion without a branch here. Recorded as declined with the CSS path still carrying it, not as a gap.
- **S4 — the leaving row collapses through `motion`.** `inbox-row.tsx` renders a `motion.div` with `initial={false}` and one `animate` target: `{ height: 0, opacity: 0 }` while leaving, `{ height: "auto", opacity: 1 }` at rest, and under `prefers-reduced-motion` the fade alone — the same split `app.css`'s reduce block has always made for this row. `useCollapseHeight` is deleted: the manual `getBoundingClientRect()` measurement and the `--row-collapse-h` custom property it wrote are gone, because `height: "auto"` → `0` is the thing `motion` does and the hand-rolled version only existed to work around `interpolate-size: allow-keywords` not being in WKWebView. Two properties moved to CSS rather than into the target, and saying so is the honest half of this entry: the row's *padding* is a layout property CSS already owns as `--row-pad-y`, so `app.css` collapses it on `.inbox-row--leaving` over the same `--dur-move` rung — two engines, two different properties, one duration, so the "no CSS transition and motion animation on the same property" rule is not in play. The data-level hold stays: `leavingIds` plus Inbox.tsx's timer exist because `react-virtuoso` owns row unmount, not because the animation needed help, and they are what the reduced-motion branch shortens to `REDUCED_FADE_MS`.
- **S5 — the three `<900` surfaces are `vaul`'s drawers.** The brief names three: the filters sheet, the thread sheet (the pane a row opens into) and the AI panel. All three are one mechanism — `packages/ui/src/components/narrow-drawer.tsx`'s `NarrowDrawer`, `Drawer.Root`/`Portal`/`Overlay`/`Content` plus a shared `DrawerGrabber` — with four load-bearing props: `handleOnly` (the drag is on the grabber alone, matching what the wide tier does by binding the gesture to the header rather than the panel), `snapPoints={[0.5, 0.92]}` (M125's rest points), `fadeFromIndex={0}` (the scrim belongs to the lowest rest point), and `autoFocus` — which is worth naming because it defaults *off*, and turning it on is what hands the modal's focus machinery back to Radix. Escape, the focus trap and the reduced-motion override are the same as they were; the override is the `!important` media-query rule at `app.css:79-83`'s pattern, applied to `[data-vaul-drawer]`'s inline transition, because `vaul` hardcodes `transform 0.5s` and reads the computed duration to decide whether it is transitioning at all.

  **S5 landed for the filters sheet, and for a while that was the whole of it.** `sheet.tsx` swapped its implementation at the breakpoint (`SheetDrawer` below 900, `SheetDialog` above) and the other two surfaces the slice names were left as they were — `.app-shell__detail`'s narrow box and `.bottom-bar .ask-panel`'s upward rule in `app.css` were untouched, and no document said which of the three had shipped. This bullet is that correction: the slice's claim is only true of the tree once all three are ported, and they are. The sheet's own shape is unchanged from the paragraph above; the pane and the panel now pass the same four props through the same component, which is what makes the second and third surfaces a reuse rather than two more implementations.

  Four consequences are worth naming, because they changed code outside the drawers:

  1. **A modal drawer owns the pointer for everything outside its content** — Radix sets `pointer-events: none` on the body — so the pane's floating action bar, which was portaled into the BottomBar's row and carried its own `THREAD_TOOLBAR_FLOATING_CLASS` and a `createPortal` call in `Thread.tsx`, would have sat visible under the scrim and untouchable. That shape is deleted; the bar is the drawer's own chrome row instead, which is what `thread-toolbar.tsx`'s `variant="chrome"` already draws at 900–1279.98 and for the same reason (the drawer's field is the material). The old narrow pane had no close affordance at all; the grabber and the scrim give it two.
  2. **Neither `vaul` nor Radix gives `Drawer.Content` a z-index**, so both new drawers carry `z-index: 40` (the sheet's own number, over the BottomBar's 30 and the pane's 20) — and the claim that the content really is above the scrim is not readable off any stylesheet, which is why the shots script hit-tests a point inside each drawer rather than documenting it in prose.
  3. **A portal is outside `#root`**, so `@container shell` cannot reach either drawer: the box each one needs — `height: 100dvh`, because `vaul`'s snap offset is a *translate* of `viewport - snap * viewport`; the window's width rather than the pane's 420px column; the glass recipe the `<=1279.98` block writes for the floating pane — is restated as a plain `[data-vaul-drawer].…` rule whose class combination *is* the media query.
  4. **The pane now opens over the shell's chrome instead of under it**, which is a behaviour change at this tier and not only a code shape. `App.tsx` auto-opens the pane on the inbox's pending-approval queue (`approvals.length > 0`, nothing selected), and below 900 that pane is modal now: its scrim covers the BottomBar, so the rail's tiles, the filters circle and the compose circle are unreachable until the drawer is put down (scrim, Escape, or the grabber). Pre-S5 the same auto-opened pane was the `z-20` sheet with the `z-30` BottomBar over it, so the bar stayed pressable while it was up. The tier keeps the behaviour it had — US-D10's line, and the drawer is the shape that behaviour now has — and both Playwright drivers had to encode the consequence: `d9-surfaces.spec.ts` closes the thread drawer before it presses Filters, and `shots-motion-oss.ts`'s `dismissPaneDrawer` is what gives the 390 rail reorder anything to press.

  One more thing the drawers' focus machinery got wrong, and this one is a configuration rather than a mechanism: `NarrowDrawer` writes focus back to whatever opened it (`useReturnFocusTarget` in front of Radix's `onCloseAutoFocus`, since none of the three is opened by a `Dialog.Trigger` Radix could find), and the AI panel is opened *by* the ask bar's input — whose `onFocus` is what opens the panel. Returning focus to it therefore reopened the panel the dismissal had just closed: at that tier Escape and the scrim were both no-ops, and the first evidence run stopped on `waiting for '[data-vaul-drawer].ask-panel' to be detached` against a drawer that was open again. `NarrowDrawer` takes `returnFocusToOpener` (default true) and the panel passes `false`, which keeps the `preventDefault` and drops the restore; `packages/ui/test/command-palette.test.tsx` holds it, and that test opens the panel the app's own way — by focusing the bar — because a drawer mounted already-open never has the input as the captured target.

  Evidence: the `sheet-open-390-*` frames (drawer top 1000px → 498px with the sheet's rows legible) and the `sheet-open-1440-*` frames (the same component as a centred dialog) for the sheet; `thread-sheet-390` and `ai-panel-390` for the other two — one still each, since what they document is a state rather than a travel, measured before they were shot (the window's width, the `0.5` snap's half-viewport top edge) and hit-tested at a point inside the drawer.
- **S6 — the toast, and `auto-animate`.** `sonner` behind a `Toaster` wrapper gives the app its first toast: "Archived · Undo" was an affordance the product did not have (`grep -rn toast` found one comment and nothing else). `auto-animate` drives the two lists whose *membership* changes on a user action — the filter chip bar and the Settings allowlist — through an exported `AnimatedList`, which exists because a hook's ref belongs to one element and Settings renders three such lists.

**Not taken: S2 — the rail's reorder stays on `pointer-drag.ts` + FLIP.** Two reasons, and the first is now measured in a browser rather than argued.

1. **`Reorder` would replace the FLIP and nothing else.** It replaces ~60 lines of layout animation. It does not replace the gesture gates, which is where the reorder's difficulty actually lives: the mouse 6px slop, the touch 350ms hold with its 8px abandon, the scroll-vs-lift disambiguation, Escape-cancel-and-revert, and click suppression after a drop — ~70 lines in `pointer-drag.ts` that the row swipe still needs whether or not `Reorder` is adopted. Porting means re-implementing those around `dragControls.start(event)`, including calling it 350ms after a press with a retained event. And the FLIP that *would* be replaced is no longer unverified: `reorder-{1440,390}-*` in `docs/design/screens/motion-oss/` drives the hand-rolled gesture at both tiers — the tile lifts, the pointer crosses the neighbour's midpoint, the neighbours travel (48px at 1440, 64px at 390) on the transition parked at 0/110/219ms, the rail order changes (`Gmail Slack System` → `Gmail System Slack`), and the lifted tile settles into the slot it took.
2. **`Reorder`'s own drag could not be driven in jsdom.** Probed: a `Reorder.Group` of three items with `getBoundingClientRect` stubbed to the same real 44px/52px stacked column `channel-rail.test.tsx` uses, driven by a full synthetic `pointerdown` → moves → `pointerup` gesture, fired `onReorder` **zero** times, with the dragged item's inline transform left at `none`; repeated with the moves dispatched on `window` in case the binding target was the issue, with the same result. Under the discarded rule that was disqualifying on its own. It is not any more — reason 1 is — but it is why a port would need its own Playwright sequence before it could be believed, for a change that deletes no dependency and no gesture code.

The row swipe is unchanged and still `pointer-drag.ts` (§c.4), narrow-tier only, with the click suppression after a release measured in Chromium and documented at the call site. This is the one gesture in the wave that is neither `motion`'s nor a library's, and it is deliberate: it is the same primitive as the rail's reorder, so the two cannot disagree about what a lift means.

**S6's two declines, with the reason corrected.** The screen switches: every screen in this shell is heavy (Inbox is a `react-virtuoso` list; Today/Network/Tasks run live Zero queries), an `AnimatePresence` swap holds the outgoing screen mounted for the length of the transition — two mounted virtualized lists, both measuring and subscribed, for a 240ms effect — and a crossfade between whole screens is the "everything fades" tell §e guards against. That decline stands. The reason recorded for it did not: the S6 commit says the brief's alternative, `AnimateView`, "is a motion+ (paid) component and is not in the MIT `motion` package". **That is false** — `AnimateView` ships in `motion@13.4.0` (re-exported from `framer-motion`'s `animate-view`), and the dependency is not paid. The real reason is a React-version one, and it is in the library's own typings: *"Requires React and React DOM 19.3 or later."* This repo is on React 18.3.1 — `apps/desktop`, `packages/ui` and the React 18 type packages all pin `^18.3.0` — so `AnimateView` is unreachable here for the same reason it is unreachable in any React 18 tree, and it becomes available the day the repo moves to React 19.3. Nothing about licensing is involved.

The chip indicator decline stands as recorded: the brief asks for shared-layout on "the category chip indicator", and no such element exists — `FilterChipBar`'s chips are removable filters with no selection state, so there is nothing to slide. The nearest real control is `SegmentedControl`, and it cross-fades deliberately; converting it to a sliding `layoutId` pill would put the label's colour flip (`[aria-checked="true"]`, instant) 160ms ahead of the pill's arrival, so the fix needs the label colour delayed or masked to the pill — more machinery than the control's existing transition. It stays a candidate rather than a port, and the honest reason it is a candidate is that the *brief's* premise was wrong, not that the library cannot do it.

**A note on the vocabulary.** This section used to end by calling `SPRING`, `TIER_MS` and `useMotionPrefs` scaffolding with no consumer in `apps/`. That is no longer true of the first and the third: `inbox-row.tsx` reads `SPRING.move` as the leave's transition, `useMotionPrefs()` for the reduced-motion branch, and `motionMs(LEAVE_MS)` for the length of the fade it falls back to. `TIER_MS` is consumed through `lib/motion.ts`'s own `PANEL_MS`/`LEAVE_MS`/`spring(tier)` rather than by name at a call site — which is what a tier vocabulary looks like when it is working, and also the reason to keep the drift test against `tokens.css` that `motion.test.tsx` holds.

The wave's evidence lives in `docs/design/screens/motion-oss/`, written by `tools/e2e/shots-motion-oss.ts`: three-frame sequences for the rail's reorder and the inbox row's leave at 1440 and 390, the sheet's opening at both tiers, and the divider drag with its band, each frame parked on the fake clock at a named millisecond and each sequence carrying an assertion the pictures alone could not make (the rail's order really changed, the row's height really fell, the drawer's top really rose, the pane really drew less than the pointer asked for). That script is the "prerequisite for revisiting" this section used to name as missing. It now exists, and it is what any future port of S2 would have to be measured against.
...
6. **No unreviewed dependency.** *Amended 2026-09-21 by the motion-OSS wave.* The five vetted animation libraries are now **allowed** (§c.1.1): `motion`, `vaul`, `@use-gesture/react`, `@formkit/auto-animate`, `sonner`. Still a reject: `dnd-kit`, a headless-UI kit, an icon pack beyond `lucide-react` / `react-icons`, GSAP, react-spring, lenis, and component kits (Aceternity, Magic UI). The rule's intent is unchanged — a dependency earns its place by doing something the primitives do badly, and it is added one at a time, with its bundle cost measured. What guard 6 was protecting against was *unvetted* dependencies, not dependencies as such.
   *Why this one was worth breaking:* the hand-rolled motion was a CSS duration ladder plus one shared pointer primitive, and the parts that needed real engineering — a spring that settles, a layout transition where siblings flow around a lifted element, an exit that unmounts only when its animation ends — were being re-derived per surface. §c.1.1 records what the wave took and what it left, with the measurement behind each. **Read it before assuming any given surface was ported: of the seven slices, five landed (S1 the runtime and presets, S3 the divider drag, S4 the row's leave, S5 the three `<900` surfaces — the filters sheet, the thread sheet and the AI panel — S6 the toast + `auto-animate`) and one was declined (S2, the rail's reorder)**, with the one remaining gesture — the row swipe — deliberately still on `pointer-drag.ts`, because it is the same primitive the rail's reorder uses and the two must not disagree about what a lift means.
