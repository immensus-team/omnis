...
Added 2026-09-21, **corrected 2026-09-21** — the first version of this section described S3, S4 and S5 as "Replaced" while they were still open, and credited `@use-gesture` with maths that `lib/detail-pane.ts` computes itself. Both were wrong. What follows is the state of the tree, slice by slice, with the measurement behind each decision.

The wave's rule, refined by the S4 probe below:

> **`motion` owns the animation layer where its *completion* is reachable in the test environment. `pointer-drag.ts` keeps the gesture layer.**

The first form of this rule said "React-level vs pointer-level", and that was too crude. S4's probe showed `AnimatePresence` *can* be driven in jsdom — so "it is a React component" is not the test. The test is narrower and more useful: an exit animation is only adoptable if the DOM change that depends on it (the unmount) happens for a reason the suite can produce. `Reorder`'s drag fails that (it needs hit-testing and layout, which jsdom does not have); `AnimatePresence` passes it only under conditions worth stating exactly (below).

**Taken:**

- **S1 — the five libraries, and the tier vocabulary.** `motion@13.4.0`, `vaul@1.1.2`, `@use-gesture/react@10.3.1`, `@formkit/auto-animate@0.10.0`, `sonner@2.0.8` (all MIT). `lib/motion.ts` gains `TIER_MS` (`base` 160 / `move` 240 / `panel` 320) and `SPRING`, which mirror the `--dur-*` rungs in `tokens.css` — with a drift test that reads `tokens.css` off disk and fails if the two ladders diverge in either direction — plus `useMotionPrefs()`, which unions `useReducedMotion()` with `prefers-reduced-transparency`. `MotionConfig reducedMotion="user"` is mounted at the root in `main.tsx`, beside the onboarding preview rather than inside `App`, so the one screen rendered *instead of* `App` is not the one place the preference is ignored.
...
**Not taken — S2, S3, S4, S5.** Each was evaluated with a probe or a reading of the library's own bundle, not by preference. The brief allows this explicitly ("explicitly recorded as not applicable with a reason"), and the reasons below are the deliverable.

**S2 — the rail's reorder stays on `pointer-drag.ts` + FLIP.**

1. **`Reorder`'s drag cannot be driven in jsdom, so the port would trade verified behaviour for unverified behaviour.** Probed directly: a `Reorder.Group` of three items, with `getBoundingClientRect` stubbed to a real 44px/52px stacked column (the same stub `channel-rail.test.tsx` uses), driven by a full synthetic `pointerdown` → moves → `pointerup` gesture. Result: `onReorder` fired **zero** times and the dragged item's inline style stayed `transform: none`. Repeated with the moves dispatched on `window` rather than the item, in case the binding target was the issue — same result. `Reorder`'s drag depends on layout, which jsdom does not have. The 13 tests that today pin the rail's reorder — slot-crossing compensation, Escape-revert, click suppression, the 350ms hold, the scroll-vs-lift disambiguation — could not follow it there.
...
**S4 — the inbox row's leave and swipe stay as they are.**

The leave probe is the one that changed this section's rule, so it is recorded in full. `AnimatePresence` *does* unmount an exited child in jsdom — but only when three things hold at once, and the third is disqualifying:

1. The test uses **real** timers. Under `vi.useFakeTimers()` the child was still mounted after advancing 2000ms past a 240ms transition, because motion drives its own `requestAnimationFrame` loop and fake timers do not step it.
2. `Element.prototype.animate` **fires `finish`**. `test/setup.ts`'s stub deliberately never does (it exists so the call is a call, not a crash), and with it the child is never unmounted. So a port would have to make that stub *lie about animations completing* for the suite to pass — a fixture that reports a finished animation that never ran.
3. **`motion` cannot express this repo's reduced-motion convention.** `reducedMotion="user"` replaces transform-driven animation with opacity but keeps the *transition's own duration*. The repo's convention is the opposite: `tokens.css` collapses `--dur-move` to 80ms under `prefers-reduced-motion`, and `app.css` swaps the row's keyframe for a plain fade at that duration. A port would hold a leaving row for 240ms under reduced motion — slower than the full-motion path it is supposed to be quieter than — and `archive-inbox.test.tsx` has a test that pins exactly that distinction (the row is held for `REDUCED_FADE_MS`, not `LEAVE_MS`). Expressed through `motion` it becomes "leaves eventually", which is not the same assertion.

Against that, what the port buys is `useCollapseHeight` — the manual `getBoundingClientRect()` measurement that writes `--row-collapse-h`, needed because `interpolate-size: allow-keywords` is not in WKWebView. That is ~20 lines of real hand-rolled physics, and it is genuinely the kind of thing `motion` is for. It is not worth the three costs above, and the `leavingIds` data-level hold survives the port regardless: it is a *view* rule (a leaving row is visible in whichever list it is leaving, `applyArchiveView`), not an animation rule.

The swipe is `pointer-drag.ts` already (§c.4), narrow-tier only, with the click-suppression after a release measured in Chromium and documented at the call site. It is pointer-driven, so it is the undrivable class above.

**S3 — the detail pane keeps its hand-rolled rubber band.**

`lib/detail-pane.ts` is pure, total and unit-tested, and its `RUBBER_BAND` is shared by the pointer drag *and* the arrow keys — `clampDetailWidth` is what a key press lands on, so substituting `@use-gesture`'s `rubberband()` for the pointer path would either split the keyboard off onto a second implementation of the same limit or move the tested maths inside the library. The spring-back is not this file's at all: it is `--ease-settle` on `.app-shell__detail`, switched off for the length of the gesture. So `@use-gesture/react` was not adopted here either; the wave installed it and did not find a place where it beat the existing maths. The collapse/expand half has the same reduced-motion problem as S4.

**S5 — the sheets stay on `sheet.tsx` + `pointer-drag.ts`.**

Measured in `vaul@1.1.2`'s own bundle, not assumed: it hardcodes `transition: transform ${TRANSITIONS.DURATION}s cubic-bezier(...)` with a `0.5s cubic-bezier(.32, .72, 0, 1)` rule on `[data-vaul-overlay]`, and it contains **no `prefers-reduced-motion` handling at all**. Adopting it would put a 500ms transform in the tree that no preference shortens — a direct breach of the wave's own "reduced-motion honoured everywhere" criterion — and overriding it means fighting inline styles for a duration the repo already owns as `--dur-panel`. On top of that, "desktop untouched" means two tier-specific implementations of a grammar `sheet.tsx` deliberately wrote once ("one component serves both, so the grammar is written once"), and its snap-and-drag is pointer- and layout-driven, i.e. the undrivable class.

**A note on the vocabulary.** `SPRING`, `TIER_MS` and `useMotionPrefs` currently have no consumer in `apps/` — the two adoptions that landed (`auto-animate`, `sonner`) bring their own timing and read `prefers-reduced-motion` themselves. They are kept rather than deleted because the brief asked for them and because `motion-prefs.test.tsx` holds a drift guard between the JS ladder and `tokens.css` that has value independent of who consumes it; the slicing above is where they would have been used. This is scaffolding, and it is called scaffolding here rather than left to look like adoption.

**The prerequisite for revisiting any of the four** is the same for all of them: a browser-driven test (Playwright, under `tools/e2e/`) that drives the gesture or the animation for real, so the behaviour has somewhere to be verified other than a jsdom suite that cannot see it. Without that, each port moves behaviour out of the only harness that currently checks it.
...
6. **No unreviewed dependency.** *Amended 2026-09-21 by the motion-OSS wave.* The five vetted animation libraries are now **allowed** (§c.1.1): `motion`, `vaul`, `@use-gesture/react`, `@formkit/auto-animate`, `sonner`. Still a reject: `dnd-kit`, a headless-UI kit, an icon pack beyond `lucide-react` / `react-icons`, GSAP, react-spring, lenis, and component kits (Aceternity, Magic UI). The rule's intent is unchanged — a dependency earns its place by doing something the primitives do badly, and it is added one at a time, with its bundle cost measured. What guard 6 was protecting against was *unvetted* dependencies, not dependencies as such.
   *Why this one was worth breaking:* the hand-rolled motion was a CSS duration ladder plus one shared pointer primitive, and the parts that needed real engineering — a spring that settles, a layout transition where siblings flow around a lifted element, an exit that unmounts only when its animation ends — were being re-derived per surface. §c.1.1 records what the wave took and what it left, with the measurement behind each. **Read it before assuming any given surface was ported: of the seven slices, two landed (the presets/runtime and the toast + `auto-animate`) and four were evaluated and declined**, so most of the hand-rolled motion above is still hand-rolled, and deliberately so.
