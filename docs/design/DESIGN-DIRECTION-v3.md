...
- **S5 — the narrow sheet is `vaul`'s drawer.** `sheet.tsx` keeps one component for both tiers and swaps the implementation at the breakpoint: `SheetDrawer` (vaul, `<900`) and `SheetDialog` (Radix, above). The drawer's four load-bearing props are `handleOnly` (the drag is on the grabber alone, matching what the wide tier does by binding the gesture to the header rather than the panel), `snapPoints={[0.5, 0.92]}` (M125's rest points), `fadeFromIndex={0}` (the scrim belongs to the lowest rest point), and `autoFocus` — which is worth naming because it defaults *off*, and turning it on is what hands the modal's focus machinery back to Radix. Escape, the focus trap and the reduced-motion override are the same as they were; the override is the `!important` media-query rule at `app.css:79-83`'s pattern, applied to `[data-vaul-drawer].sheet`'s inline transition, because `vaul` hardcodes `transform 0.5s` and reads the computed duration to decide whether it is transitioning at all. Evidence: the `sheet-open-390-*` frames (drawer top 1000px → 498px with the sheet's rows legible) and the `sheet-open-1440-*` frames (the same component as a centred dialog).
