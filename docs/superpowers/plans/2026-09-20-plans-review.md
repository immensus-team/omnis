...
| M7 | adapters T4 vs desktop T9 | one Keychain scheme | Adapters read `omnis.slack.xoxb.<team>` (account=`<team>`) + `…xoxb.<team>.app`, 2 entries. Onboarding writes only `omnis.slack.xoxp.<team>` (account=`omnis`), 1 entry → connect fails | desktop T9 |
