...
| 3 | draft-worthiness | `loops/draft.ts` trigger `needs_reply_score >= 0.5` | `worth_drafting` (boolean), `register` (choice) | Only ever **lowers** the gate. Jev saying "not worth drafting" skips the loop; Jev saying "worth it" changes nothing, because writing is an LLM job. |
| 4 | follow-up nudge | `loops/followup.ts` (`sweepFollowups` → `person.inactive`) | `reach_out` (boolean), `kind` (choice: post_meeting/first_contact/dormant_revive/pending_step) | Same veto-only shape as #3. `pickFollowupChannel` and the `NO_COLD_OUTREACH_CHANNELS` gate are untouched and run afterwards. |
| 5 | delegation eligibility | `delegate/route.ts` `routeByRule` → L4 (LLM) | `delegate` (boolean), `runtime` (choice: omnis/codex/claude_code/claude_ds) | Only consulted when `routeByRule` returned `null`. `pickRuntime`'s deterministic table still wins where it applies, and `DELEGATION_DAILY_CAP` / `DELEGATION_THREAD_CAP_24H` are enforced outside it. |
