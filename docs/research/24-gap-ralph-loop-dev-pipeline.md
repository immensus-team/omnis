...
omnis의 개발 파이프라인은 새로 설계할 게 아니라 이미 이 머신에 깔린 두 로컬 도구를 합치면 된다: oh-my-claudecode의 `ralph` skill(PRD 기반 story loop + tiered reviewer + 강제 deslop/regression 재검증)과 `claude-squad`/`worktrunk`의 per-task git worktree 격리. 여기에 DeepSeek 티어 하나만 추가하면 된다: task는 "files + acceptance criteria + verify command"로 self-contained하게 쪼개서 `claude-ds -p ...`로 위임. **가장 중요한 신규 발견**: "Fable"은 브리프의 별명이 아니라 Claude Code의 실제 model alias(`opus` 위의 최상위 티어)이며, headless(`-p`)/Agent SDK 모드에서는 usage-credit 소비에 대한 동의 프롬프트가 **뜨지 않고 그냥 과금**된다 — 따라서 무인 ralph 루프에는 `fable`을 절대 기본 워커로 넣으면 안 되고, planning처럼 Logan이 지켜보는 interactive 세션에만 예약해야 한다. ToS 경계는 이미 검증됨: unmodified `claude` 바이너리를 서브프로세스로 호출하는 건 되지만, OAuth 토큰을 Agent SDK로 우회 사용하는 건 명시 금지.
...
- **Fable:** planning/PRD-refinement only, run interactively so the usage-credit consent prompt actually fires. Never the default headless build worker — this matches both the ToS-adjacent silent-billing risk found this session and Logan's own brief: "절대 fable로 모든 일들을 다 처리하려고 하지마."
