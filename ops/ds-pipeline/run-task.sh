#!/bin/bash
# DeepSeek-only implement→review loop. Usage: run-task.sh <worktree> <task.md> [max_attempts] [extra_verify_cmd]
set -u
...
$(cat "$TASK")$ISSUES" --permission-mode bypassPermissions --strict-mcp-config --output-format json > "$LOG/$NAME-impl-$a.json" 2> "$LOG/$NAME-impl-$a.err"
  ...
  [ -n "$EXTRA" ] && V="$V extra=$(bash -c "$EXTRA" >/dev/null 2>&1 && echo pass || echo FAIL)"
  echo "[$(date +%H:%M:%S)] $NAME attempt $a: verify $V"
  rm -f .omnis-review.json
  gtimeout 1800 claude-ds -p "$COMMON

# ROLE: independent, strict code/design REVIEWER (you did not write this code). Worktree: $WT. Base: $BASE (git merge-base with main).
Task under review:
$(cat "$TASK")
